...
type DayEntry = {
    ...
    label: string;                // อา./จ. 01/10
    total: number;                // sum signed
    items: Item[];
};
...
function itemToForm(it: Item): EditForm {
    return {
        typeLabel: it.type === "EXPENSE" ? "รายจ่าย" : "รายรับ",
...
export default function Summary() {
        ...
        setEntries((prev) =>
            prev
                .map((d) => ({
                    ...
                    total: d.items.filter((x) => x.id !== id).reduce((s, it) => s + it.amount, 0),
                }))
    ...
    const tryDeleteEndpoints = async (id: number) => {
        ...
        if (await attempt(`/api/expenses/${id}`, { method: "DELETE", credentials: "include", headers: { Accept: "application/json" } })) return true;
        if (await attempt(`/api/expenses/${id}?_method=DELETE`, { method: "POST", credentials: "include", headers: { Accept: "application/json" } })) return true;
        if (await attempt(`/api/expenses/delete/${id}`, { method: "POST", credentials: "include", headers: { Accept: "application/json" } })) return true;
        if (await attempt(`/api/expenses/${id}`, { method: "DELETE", credentials: "include" })) return true;
        return false;
    };
    ...
    const onDelete = async (it: Item) => {
        ...
        try {
            ...
            if (!ok) throw new Error("ลบรายการไม่สำเร็จ");
    ...
    const submitEdit = async () => {
        ...
        const apiType: "EXPENSE" | "INCOME" = form.typeLabel === "รายรับ" ? "INCOME" : "EXPENSE";
        const payload = {
            id: selected.id,
            type: apiType,
        ...
        try {
            setSaving(true);
            let res = await fetch(`/api/expenses/${selected.id}`, {
                method: "PUT",
                credentials: "include",
            ...
            if (!res.ok) {
                ...
                const delRes = await fetch(`/api/expenses/${selected.id}`, {
                    method: "DELETE",
                    credentials: "include",
                    headers: { Accept: "application/json" }
                });
            ...
            alert(e?.message || "เกิดข้อผิดพลาดระหว่างบันทึก");
        } finally {
            setSaving(false);
        }
    };
        ...
        return (
            <div className="App summary-page">
                <div className="list-wrap">
                    <div className="day-card">กำลังดึงข้อมูล…</div>
                </div>
                <BottomNav />
            </div>
        );
    ...
    if (error) {
        return (
            <div className="App summary-page">
                <div className="list-wrap">
                    <div className="day-card neg">{error}</div>
                </div>
                <BottomNav />
            </div>
        );
                ...
                {entries.length === 0 && (
                    <section className="day-card">
                        <header className="day-header">
                            <span className="day-date">ยังไม่มีรายการ</span>
                        </header>
                            ...
                            <div className="detail-body">
                                <div className="edit-grid">
                                    <div className="pair">
                                        <div className="field">
                                            <label>ประเภท</label>
                                            <input readOnly value={selected.type === "INCOME" ? "รายรับ" : "รายจ่าย"} />
                                        </div>
                                        <div className="field">
