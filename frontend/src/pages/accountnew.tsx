...
const ICONS = [
    ...
    { key: "wallet", label: "กระเป๋าสตางค์", Icon: Wallet },
...
export default function AccountNew() {
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (!name.trim() || !type || Number.isNaN(amt)) {
            alert("กรอกข้อมูลให้ครบถ้วน และจำนวนเงินให้ถูกต้อง");
            return;
        }
        const nextItem: Account = {
            name: name.trim(),
            amount: amt,
            iconKey,
    ...
    return (
        <div className="accnew-wrap">
            <h1 className="title">
                {editState?.mode === "edit" ? "แก้ไขบัญชี" : "สร้างบัญชี"}
            </h1>
            ...
            <span className={type ? "" : "placeholder"}>
              {type || "ประเภท"}
                ...
                {/* จำนวนเงินตั้งต้น */}
                <label className="row">
                    <span className="label">จำนวนเงินตั้งต้น</span>
