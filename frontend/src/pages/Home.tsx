...
function loadAccounts(): Account[] {
    ...
    return [
        { name: "เงินสด", amount: 20000, iconKey: "bank" },
...
export default function Home() {
    ...
    const handleDelete = (idx: number) => {
        ...
        const ok = window.confirm(`ลบบัญชี “${acc.name}” ?`);
        if (!ok) return;
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                // ใช้ api.get เพื่อให้ BASE /api จาก api.ts ทำงานถูกในทุกสภาพแวดล้อม
                const json = await api.get<ExpenseDTO[]>(`/expenses/range?start=${start}&end=${end}`);
                if (!alive) return;
                setState({ loading: false, error: null, data: json });
            } catch (e: any) {
                if (!alive) return;
                setState({ loading: false, error: e?.message || "ดึงข้อมูลไม่สำเร็จ", data: [] });
            }
        })();
        return () => { alive = false; };
    }, [year, month]);
    ...
    // ยอดกระเป๋าโดยรวม = รวมบัญชี + รายรับเดือนนี้ - รายจ่ายเดือนนี้
            ...
            <div className="main-content">
                ...
                <div className="balance-card">
                    <div className="balance-display">
                        <p className="balance-label">ยอดรวม</p>
                        <p className="balance-amount">{formatTH(walletBalance)} บาท</p>
                    </div>

                    <div className="month-year-nav">
                        <button onClick={prevMonth} aria-label="เดือนก่อนหน้า" className="arrow-btn">←</button>
                        <span>{month}/{year}</span>
                        <button onClick={nextMonth} aria-label="เดือนถัดไป" className="arrow-btn">→</button>
                    </div>
                ...
                <span style={{ fontSize: 18, fontWeight: "bold", color: monthBalance < 0 ? "#ef4444" : "#16a34a" }}>
                  {monthBalance < 0 ? "-" : ""}{formatTH(Math.abs(monthBalance))}
                            ...
                            <span className="action-label">คงเหลือ</span>
                        </button>

                        <button className="action-button" title="รายรับรวมของเดือน">
                            <div className="action-icon">
                <span style={{ fontSize: 18, fontWeight: "bold", color: "#16a34a" }}>
                  {formatTH(monthIncome)}
                </span>
                            </div>
                            <span className="action-label">รายรับ</span>
                        </button>

                        <button className="action-button" title="รายจ่ายรวมของเดือน">
                            <div className="action-icon">
                <span style={{ fontSize: 18, fontWeight: "bold", color: "#ef4444" }}>
                  -{formatTH(monthExpense)}
                </span>
                            </div>
                            <span className="action-label">รายจ่าย</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="transaction-header">
                    ...
                    <Link to="/summary" className="transaction-link">ดูสรุปทั้งหมด</Link>
                </div>

                {loading && <div className="transaction-empty">กำลังดึงข้อมูล…</div>}
                {error && !loading && <div className="transaction-empty neg">{String(error)}</div>}
                {!loading && !error && recent.length === 0 && (
                    <div className="transaction-empty">ยังไม่มีรายการของเดือนนี้</div>
                )}
