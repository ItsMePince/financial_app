        ...
        (async () => {
            try {
                ...
                const res = await fetch(
                    `/api/expenses/range?start=${dayISO}&end=${dayISO}`,
                    { headers: { Accept: "application/json" }, credentials: "include" }
                );
                if (!res.ok) throw new Error(`ดึงรายการไม่สำเร็จ (${res.status})`);
                const data: ExpenseDTO[] = await res.json();
                if (!alive) return;
                setRaw(data);
            } catch (e: any) {
                if (!alive) return;
                setError(e?.message || "เกิดข้อผิดพลาดในการดึงข้อมูลจากเซิร์ฟเวอร์");
            } finally {
            ...
            <section className="day-card">
                <div className="card-head">
                    <button
                        ...
                        aria-label="กลับไปหน้าสรุปรายเดือน"
                        title="กลับไปหน้าสรุปรายเดือน"
                    >
                        <ArrowLeft size={21} />
                        <span>กลับ</span>
                    </button>
                ...
                <div className="switcher">
                    <button className="nav-btn" onClick={prevDay} aria-label="ก่อนหน้า" title="ก่อนหน้า">←</button>
                    <div className="date-chip">
                        <CalendarDays size={15} /> {thDate(anchor)}
                    </div>
                    <button className="nav-btn" onClick={nextDay} aria-label="ถัดไป" title="ถัดไป">→</button>
                </div>
                    ...
                    items.map((it) => {
                        ...
                        return (
                            <div className="item" key={it.id}>
                                ...
                                <div className="amount">{it.amount.toLocaleString("th-TH")} ฿</div>
                            </div>
                        );
                    })
                )}
