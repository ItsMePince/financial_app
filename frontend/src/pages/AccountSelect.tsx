...
const initialItems: AccountItem[] = [
    ...
    { id: "ktc",   label: "บัตร KTC",      type: "credit", favorite: false },
];
...
export default function AccountSelect() {
    ...
    const setPayment: any = pm?.setPayment ?? (() => {});
    ...
    const toggleFavorite = (id: string) => {
        setItems((prev) => {
            ...
            return prev.map((it) =>
                it.id === id
                    ? { ...it, favorite: !it.favorite, favoritedAt: !it.favorite ? now : undefined }
                    : it
            );
        });
    };
    ...
    const gotoAddPage = () => alert("หน้าสร้างบัญชี/วิธีชำระเงิน (ยังไม่ทำ)");
    ...
    return (
        <div className="screen">
            <div className="dropdown">
                ...
                {menuOpen && (
                    <ul className="dropdown__menu" role="listbox" onMouseLeave={() => setMenuOpen(false)}>
            ...
            <div className="list">
                {sortedAndFiltered.length === 0 ? (
                    ...
                    sortedAndFiltered.map((it) => (
                        <button key={it.id} className="card" onClick={() => pick(it)}>
