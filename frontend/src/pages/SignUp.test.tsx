...
describe("SignUp (Frontend only)", () => {
  ...
  it("validate: รหัสผ่านต้องยาวอย่างน้อย 6 ตัวอักษร", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /create account/i }));
    expect(await screen.findByText(/อย่างน้อย 6 ตัวอักษร/i)).toBeInTheDocument();
  });
  ...
  it("เส้นทาง onSubmit prop: เรียก callback ด้วยค่าฟอร์ม และไม่เรียก fetch", async () => {
    ...
    // สร้างสปาย (ถ้าไม่มี fetch ใน env เดิม ให้ผูกเป็นฟังก์ชันว่างก่อน)
if (!(global as any).fetch) {
  (global as any).fetch = jest.fn();
}
const fetchSpy = jest.spyOn(global as any, "fetch");

// ...render + กด submit ตามเดิม...
...
expect(fetchSpy).not.toHaveBeenCalled();
  ...
  it("API success (ไม่มี onSignUpSuccess): redirect ไป /Home", async () => {
    ...
    // mock window.location.href (read-only): สร้าง property ชั่วคราว
  ...
  it("Network error: แสดงข้อความภาษาไทยตามที่กำหนด", async () => {
    (global.fetch as any) = jest.fn().mockRejectedValueOnce(new Error("Network down"));
